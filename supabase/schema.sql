...
create table if not exists public.profiles (
  ...
  name text,
  email text,
  role text not null default 'user' check (role in ('admin', 'user', 'partner', 'custom')),
...
-- Partner briefing layer. This block is also available as an incremental
...
-- Push notification devices and delivery queue. The incremental source for
-- existing projects is supabase/migrations/add_push_notifications.sql.
...
create index if not exists push_device_tokens_active_user_idx on public.push_device_tokens(user_id, last_seen_at desc) where revoked_at is null;
create index if not exists push_notification_deliveries_pending_idx on public.push_notification_deliveries(status, created_at asc) where status = 'pending';
...
create trigger push_device_tokens_set_updated_at before update on public.push_device_tokens for each row execute function public.set_push_updated_at();
drop trigger if exists push_notification_deliveries_set_updated_at on public.push_notification_deliveries;
create trigger push_notification_deliveries_set_updated_at before update on public.push_notification_deliveries for each row execute function public.set_push_updated_at();
...
create policy "push_device_tokens_select_own" on public.push_device_tokens for select using (auth.uid() = user_id);
create policy "push_device_tokens_insert_own" on public.push_device_tokens for insert with check (auth.uid() = user_id);
create policy "push_device_tokens_update_own" on public.push_device_tokens for update using (auth.uid() = user_id) with check (auth.uid() = user_id);
create policy "push_device_tokens_delete_own" on public.push_device_tokens for delete using (auth.uid() = user_id);
drop policy if exists "push_deliveries_select_recipient" on public.push_notification_deliveries;
create policy "push_deliveries_select_recipient" on public.push_notification_deliveries for select using (auth.uid() = user_id);

create or replace function public.register_push_device_token(registration_token text, registration_platform text default 'android', registration_device_label text default null)
returns public.push_device_tokens language plpgsql security definer set search_path = public as $$
declare registered_token public.push_device_tokens;
begin
  if auth.uid() is null then raise exception 'Sessao ativa necessaria para registrar notificacoes'; end if;
  if coalesce(length(trim(registration_token)), 0) < 20 then raise exception 'Token de notificacao invalido'; end if;
  if registration_platform <> 'android' then raise exception 'Plataforma de notificacao invalida'; end if;
  ...
  on conflict (token) do update set user_id = excluded.user_id, platform = excluded.platform, device_label = excluded.device_label, last_seen_at = now(), revoked_at = null
...
begin
  if auth.uid() is null then raise exception 'Sessao necessaria para remover notificacoes'; end if;
  update public.push_device_tokens set revoked_at = now(), last_seen_at = now() where user_id = auth.uid() and token = trim(registration_token);
end;
$$;
...
begin
  ...
  values (new.id, new.user_id, case when new.lead_id is null then '/partner' else concat('/leads/', new.lead_id::text, '/briefing') end)
...
create trigger partner_notifications_enqueue_push after insert on public.partner_notifications for each row execute function public.enqueue_partner_notification_push();

alter table public.partner_notifications drop constraint if exists partner_notifications_type_check;
alter table public.partner_notifications add constraint partner_notifications_type_check check (type in ('visit_briefing_assigned', 'visit_briefing_updated', 'visit_assigned', 'visit_rescheduled', 'visit_status_updated', 'feedback_requested', 'deadline_approaching', 'partner_feedback_received')) not valid;

create or replace function public.notify_lead_owner_partner_feedback()
returns trigger language plpgsql security definer set search_path = public as $$
begin
  if new.partner_visit_feedback is distinct from old.partner_visit_feedback and nullif(trim(coalesce(new.partner_visit_feedback, '')), '') is not null and new.user_id is not null and new.user_id is distinct from new.partner_id then
    insert into public.partner_notifications (user_id, lead_id, type, title, body)
    values (new.user_id, new.id, 'partner_feedback_received', 'Retorno de parceiro recebido', concat('O parceiro registrou o retorno da visita de ', new.name, '.'));
  end if;
  return new;
end;
$$;
...
create trigger leads_notify_owner_partner_feedback after update of partner_visit_feedback on public.leads for each row execute function public.notify_lead_owner_partner_feedback();
